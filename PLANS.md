...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, C2 is complete at `checkpoint/20260711-c2-artifact-evidence`, and C3b is complete at `checkpoint/20260711-c3b-resident-control-plane`. C5 is active under `docs/plans/2026-07-11-c5-production-capability-relocation.md`. Tasks 1 through 3 are complete. The Lab-owned developer-only `actingcommand-resource-tooling` boundary owns deterministic resource compilation and package build behind typed snapshots. Pure task/probe planning and its 37 established tests now belong to execution-kernel without device side effects; device-test consumes the new owner directly, and the unreferenced task-loop crate is retired from source, metadata, and lock state after pre/post-removal equivalence gates. Task 4 is active: subtask 4a moved pure read-only target/page recognition and its response models into `ReadonlyRecognitionEngine`. Subtask 4b1 moved environment result DTOs plus freshness, confidence/expiry/value validation, marker collection, and recursive marker resolution into `EnvironmentStateEngine`. Subtask 4b2a moved structured/flat catalog normalization, schema/key/step/candidate validation, detector selection, scope canonicalization, ROI decoding, and device-free step plans into execution-kernel; Lab retains file I/O, local identity/persistence, template observations, and effect adapters. Subtask 4b2b will move candidate observation selection and result construction; subtask 4c will replace production Lab capture construction with daemon-owned observation requests. Later C5 tasks move drive/run/recovery, keep frame/output authority in artifact-store and ledger, move Session/monitor/stream live state into Runtime host, and reduce Lab to optional authoring/debug/sealed adapters. C6 retains full authoring workflow and transactional publication. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
