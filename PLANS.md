...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, C2 is complete at `checkpoint/20260711-c2-artifact-evidence`, and C3b is complete at `checkpoint/20260711-c3b-resident-control-plane`. C5 is active under `docs/plans/2026-07-11-c5-production-capability-relocation.md`. Tasks 1 and 2 are complete. The Lab-owned developer-only `actingcommand-resource-tooling` boundary now owns package API DTOs, validation, conversion, MAA compilation, and package build. Package build uses a two-phase prepare/build API: Lab resolves required environment facts once and passes an immutable typed snapshot, while the compiler has no Lab, live config, Runtime, scheduler, execution-kernel, or device access. Thin Lab wrappers preserve all existing callers and protocol envelopes. Architecture guards reject production reachability to resource-tooling and reject resource-tooling access to live owners. Task 3 is active. Subtask 3a moved the pure task/probe planning implementation and its 37 established tests into execution-kernel without device side effects; device-test now consumes execution-kernel directly, and full-workspace equivalence passed while task-loop remained a narrow compatibility shell. Subtask 3b will remove that unreferenced shell and repeat retirement gates. Later C5 tasks move env/recognition/drive/run/recovery into production execution modules, keep frame/output authority in artifact-store and ledger, move Session/monitor/stream live state into Runtime host, and reduce Lab to optional authoring/debug/sealed adapters. C6 retains full authoring workflow and transactional publication. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
