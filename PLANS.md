...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, and C4 is complete under `docs/plans/2026-07-11-c4-minimal-vertical-slice.md`. C4 delivers a resource-independent frame observation as the read-only request and `input.reset` as the safe control request. Thin `actingctl` and ActingLab adapters use the same correlation-scoped runtime-client flow, host-issued receipt, and ledger projection. Capture remains client-side and read-only under the C3a exception; every write remains scheduler-fenced behind daemon DeviceProxy. C4 does not add queueing, priority, preemption, task lifecycle, daemon capture, execution-kernel ownership, remote API, UI, game logic, or C2 artifact behavior. C2 remains independently ready, and C3b is the next critical-path node when selected. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
