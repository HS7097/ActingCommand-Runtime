...
Current phase: C1 global event contract and ledger skeleton hardening. Tasks 1-5 of the original C1 plan are complete. The whole-branch review found 2 Critical, 8 Important, and 3 Minor findings, and Alice approved strengthening C1 rather than narrowing its guarantees. Hardening Tasks 1-6 are complete. Event/fact authority, post-action critical outcomes, bounded absorbing subscriptions, and owned startup are hardened. Final-tail repair now persists and syncs a deterministic prepared record before quarantine or truncation, uses deterministic quarantine and recovery-event identities, resumes idempotently from every crash boundary, and records completion only after the verified recovery fact is durable. Writer metadata is v2; unapproved v1 writer/event roots fail explicitly. Hardening Task 7, adversarial C1 acceptance and closeout, is next. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
...
Current implementation: C1 Tasks 1-5 are complete and task-scoped review is approved: typed event/query/projection contracts, pre-persistence redaction, recoverable single-writer append storage, correlated ordered queries, recovered indexes, replay-plus-live subscriptions with bounded nonblocking live delivery, separate CLI/UI/Lab projections, validated critical intent/action/outcome ordering, hard-kill recovery proof, correlated five-source acceptance, redaction boundary coverage, and all-features dependency-law guards. The approved hardening replaces producer-selected classifications with schema-owned typed payloads, moves persisted facts into the ledger, completes artifact references, builds outcomes after actions, bounds replay, owns startup synchronously, journals destructive repair, and closes remaining diagnostic/storage invariants before C1 may close.
