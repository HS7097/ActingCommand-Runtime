...
- M6 retained-frame false-positive calibration is partially closed for the available AK corpus. Runtime `resource convert` now supports source `page_rules`, and the AK resource pack adds retained mission-result and terminal-stage targets plus page rules so current home frames match only `arknights/home`, the retained mission-result frame matches no generated page, and the retained terminal stage map matches only `arknights/terminal`.
...
- M6 threshold calibration/reporting remains incomplete for several destination-page positives. The available AK retained corpus proves the home, mission-result, and terminal-stage false-positive repair, and the operator page no longer has mutually exclusive required anchors, but the corpus still does not contain positive retained frames for `recruit`, `depot`, `friends`, `gacha`, `infrast`, `mall`, `mission`, `operator`, or the actual QuickSwitch dropdown overlay.
