...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`; C3a resident Runtime seed implementation is active under `docs/plans/2026-07-11-c3a-runtime-seed.md`. C3a Tasks 1-6 are complete: the contract owns typed Runtime requests/receipts, fencing lease tokens, errors, discovery info, read-only capture capability, and event links; `actingcommand-scheduler` owns typed per-instance lease admission, monotonic expiry, idempotent renew/release, connection ownership, takeover cooldown, and full pre-write fencing decisions; `actingcommand-runtime-host` owns the resident owner guard, GlobalLedger writer, loopback typed IPC, durable owner metadata, scheduler/backend lifecycle, and lease-gated DeviceProxy; `actingcommand-runtime-client` plus thin `actingd` provide discovery, correlated typed requests, bounded heartbeat-backed input proxying, typed process configuration, nonzero fatal exit mapping, and independent daemon/client lifetimes; ActingLab production input crosses only the Runtime proxy while dry-run, sealed fake input, and client-side read-only capture retain their C3a boundaries; and adversarial/process acceptance proves deterministic same-instance contention, independent instances, complete fencing, hard-kill takeover with old-token rejection, cooldown enforcement, client-drop cleanup, and one correlated ledger sequence. Task 7 whole-C3a closeout is in progress: fresh review corrected renew/release replay after bounded-cache loss and separated host fatality from client fallback eligibility. C3a does not add queueing, priority, preemption, task lifecycle, daemon capture, execution-kernel ownership, remote API, UI, or game logic. C2 remains independently ready. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
