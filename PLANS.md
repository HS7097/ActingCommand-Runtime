...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, and C2 is complete at `checkpoint/20260711-c2-artifact-evidence`. C3b is active under `docs/plans/2026-07-11-c3b-control-plane.md`. Tasks 1-3 are complete: the typed queue/preemption/transfer contract and scheduler core, the daemon-only `actingcommand-execution-kernel`, and Runtime-host queue/control-plane integration now form one resident input path. Queue admission, poll, cancellation, expiry, disconnect, idle and post-input preemption, durable transfer-before-authority ordering, stale-token fencing, and per-instance independence are ledger-visible and tested. Backend sessions remain resident until Runtime shutdown; client lease release does not close them. Task 4 is active and will retire the C3a client-side capture exception by moving read-only capture into the daemon, removing client device/recognition dependencies, and adding the final process/dependency hard gates. C5 remains responsible for migrating production recognition, drive, run, recovery, and task capabilities. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
