...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, C2 is complete at `checkpoint/20260711-c2-artifact-evidence`, and C3b is complete at `checkpoint/20260711-c3b-resident-control-plane`. The resident control plane owns typed queue/preemption/transfer, daemon-only input and capture sessions, Runtime-host queue integration, durable transfer-before-authority ordering, stale-token fencing, and per-instance independence. Read-only observation is one daemon-side operation, actingd requires explicit input and capture backends, and production runtime-client/actingctl code has no device or recognition dependency or backend constructor path. Backend sessions remain resident until Runtime shutdown; client lease release does not close them. The next approved phase is C5: migrate production recognition, drive, run, recovery, task capabilities, and the existing frame-store behavior into their frozen production ownership boundaries without making Runtime an authoring tool. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
