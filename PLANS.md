...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, and C2 is complete at `checkpoint/20260711-c2-artifact-evidence`. C3b is active under `docs/plans/2026-07-11-c3b-control-plane.md`. Tasks 1-2 are complete: the typed queue/preemption/transfer contract and scheduler core are followed by `actingcommand-execution-kernel`, a daemon-only, lazily opened input/capture session shell. A backend error terminates and latches the session without reconnect, retry, fallback, or fake success; close and panic failures remain explicit while private device details stay out of error surfaces. Architecture guards restrict the new crate to contract/device dependencies and allow only Runtime host/actingd as production consumers. Task 3 now integrates this authority, queue operations, safe-yield transfer, and ledger ordering into Runtime host. The C3a client-side capture exception will be retired in Task 4. C5 remains responsible for migrating production recognition, drive, run, recovery, and task capabilities. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
