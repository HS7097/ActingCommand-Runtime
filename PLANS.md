...
Current phase: C1 global event contract and ledger skeleton hardening is complete. The whole-branch review originally found 2 Critical, 8 Important, and 3 Minor findings, and Alice approved strengthening C1 rather than narrowing its guarantees. Hardening Tasks 1-7 now close event/fact authority, post-action critical outcomes, bounded absorbing subscriptions, owned startup, crash-atomic final-tail repair, adversarial secret/correlation/process acceptance, and all-features Lab-removability guards. The final direct review found one additional Important lifecycle defect: a locally latched terminal subscription could retain its writer registration. Commit `ad64d8f` detaches terminal subscriptions, and the repeated C1 review has no remaining Critical or Important finding. Event schema is `actingcommand.event.v2`, writer metadata is `actingcommand.ledger-writer.v2`, and the repair journal is `actingcommand.ledger-repair.v1`; unapproved generic v1 event roots and v1 writer metadata fail explicitly. The next critical-path node is C3a, while C2 remains independently ready. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
