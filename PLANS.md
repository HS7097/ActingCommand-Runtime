...
Current phase: C1 is complete at `checkpoint/20260711-c1-ledger-hardening`, C3a is complete at `checkpoint/20260711-c3a-runtime-seed`, C4 is complete at `checkpoint/20260711-c4-minimal-vertical-slice`, and C2 is active under `docs/plans/2026-07-11-c2-artifact-evidence.md`. C2 Tasks 1-4 are complete: the production artifact authority and durable store exist, the accepted frame-store mechanics now live in artifact-store behind a narrow Lab compatibility facade, Tier1/Tier2/Tier3 behavior remains covered, policy/dedup/pressure/pinned-failure facts plus lossless pinned persistence are enforced, and the shared evidence exporter produces atomically published, independently reopened and rehashed archives for success/failure/cancelled outcomes. The archived manifest hashes every declared entry; the typed receipt and attached archive reference carry the real non-self-referential final ZIP hash. C2 Task 5, sealed acceptance through the real GlobalLedger ingress/query/projection path, is next. C2 does not move capture into the daemon, add execution-kernel ownership, or enter C3b/C5/C6 behavior. C3b remains the next critical-path node after C2 closeout. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
