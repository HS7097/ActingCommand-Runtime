...
Current phase: C1 global event contract and ledger skeleton hardening. Tasks 1-5 of the original C1 plan are complete. The whole-branch review found 2 Critical, 8 Important, and 3 Minor findings, and Alice approved strengthening C1 rather than narrowing its guarantees. Hardening Tasks 1-3 are complete. The event v2 and opaque-ledger-fact migration closes producer and persisted-fact authority, while the critical executor now appends intent before invoking an action and builds, sanitizes, validates, and durably appends only the actual post-action outcome. Panic propagation is not intercepted, outcome-undurable failures preserve the known effect without returning a receipt, and C1 makes no cross-call exactly-once claim. Hardening Task 4, bounded replay and absorbing subscription state, is next. Alice approved the complete Issue #35 chain and clarified that C0-C7 nodes do not require separate approval. The approved C0 architecture is `docs/architecture/runtime-ledger-v3-c0-freeze.md`, payload SHA-256 `6c72a9c39ff67ec5a2868e0ed262d2a2f0a2c4b0fbfc473b7c55a9df610bf0a7`.
