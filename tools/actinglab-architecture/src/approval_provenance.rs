...
pub fn verify_approval_provenance(
    ...
    validate_active_change_binding_range(
        &base,
        &head,
        &verified_approvals,
        |ancestor, descendant| commit_is_ancestor(&root, ancestor, descendant),
    )?;
    ...
    require_nonzero_surface_changes(surface_changes_verified)?;
...
fn validate_active_change_binding_range(
    base: &str,
    head: &str,
    approvals: &HashMap<String, VerifiedApproval>,
    mut is_ancestor: impl FnMut(&str, &str) -> Result<bool, String>,
) -> Result<(), String> {
    for (id, verified) in approvals {
        if verified.authority != ApprovalAuthority::Active {
            continue;
        }
        let binding = verified
            ...
            .change_binding
            .as_ref()
            .ok_or_else(|| format!("active approval {id} has no immutable change binding"))?;
        if !binding.base_sha.eq_ignore_ascii_case(base) {
            return Err(format!(
                "approval {id} change binding base {} does not match actual base {base}",
                binding.base_sha
            ));
        }
        let subject = binding.subject_sha.to_ascii_lowercase();
        if subject == base || is_ancestor(&subject, base)? {
            return Err(format!(
                "approval {id} change binding subject {subject} is already contained in base {base}"
            ));
        }
        if !is_ancestor(base, &subject)? {
            return Err(format!(
                "approval {id} change binding subject {subject} is not descended from base {base}"
            ));
        }
        if !is_ancestor(&subject, head)? {
            return Err(format!(
                "approval {id} change binding subject {subject} is not reachable from head {head}"
            ));
        }
    }
    Ok(())
...
fn require_nonzero_surface_changes(surface_changes: usize) -> Result<(), String> {
    if surface_changes == 0 {
        Err("approval provenance cannot verify a zero-surface base-to-head range".to_string())
    } else {
        Ok(())
    }
}
...
fn ensure_ancestor(root: &Path, base: &str, head: &str) -> Result<(), String> {
    if commit_is_ancestor(root, base, head)? {
        Ok(())
    } else {
        Err(format!(
...
fn commit_is_ancestor(root: &Path, ancestor: &str, descendant: &str) -> Result<bool, String> {
    let output = Command::new("git")
        .arg("-C")
        .arg(root)
    ...
    Err(format!(
        "failed to verify commit ancestry {ancestor}..{descendant}: {}",
        String::from_utf8_lossy(&output.stderr).trim()
    ))
}
...
mod tests {
    ...
    #[test]
    fn active_binding_covers_exact_base_subject_head_range() {
        let base = "a".repeat(40);
        let subject = "b".repeat(40);
        let head = "c".repeat(40);
        let approval = active_approval(&["approval.provenance", "surface.mapping"]);
        let verified =
            bind_approval_authority(HashMap::from([(approval.id.clone(), approval)])).unwrap();
        let ancestry = |ancestor: &str, descendant: &str| {
            Ok(ancestor == descendant
                || ancestor == base && (descendant == subject || descendant == head)
                || ancestor == subject && descendant == head)
        };

        validate_active_change_binding_range(&base, &head, &verified, ancestry).unwrap();

        let wrong_base = "d".repeat(40);
        let error =
            validate_active_change_binding_range(&wrong_base, &head, &verified, |_, _| Ok(false))
                .unwrap_err();
        assert!(error.contains("does not match actual base"));

        let mut subject_in_base = verified.clone();
        let entry = subject_in_base.get_mut("approval.test_active").unwrap();
        entry.approval.change_binding.as_mut().unwrap().base_sha = subject.clone();
        let error = validate_active_change_binding_range(
            &subject,
            &head,
            &subject_in_base,
            |ancestor, descendant| Ok(ancestor == descendant || descendant == head),
        )
        .unwrap_err();
        assert!(error.contains("already contained in base"));
        ...
        let error = validate_active_change_binding_range(
            ...
            &verified,
            |ancestor, descendant| {
                Ok(ancestor == descendant || ancestor == subject && descendant == head)
            },
        )
        .unwrap_err();
        assert!(error.contains("not descended from base"));
        ...
        let error = validate_active_change_binding_range(
            ...
            &verified,
            |ancestor, descendant| {
                Ok(ancestor == descendant || ancestor == base && descendant == subject)
            },
        )
        .unwrap_err();
        assert!(error.contains("not reachable from head"));
    }

    #[test]
    fn zero_surface_range_cannot_report_verified() {
        assert!(require_nonzero_surface_changes(1).is_ok());
        let error = require_nonzero_surface_changes(0).unwrap_err();
        assert!(error.contains("zero-surface"));
    }
