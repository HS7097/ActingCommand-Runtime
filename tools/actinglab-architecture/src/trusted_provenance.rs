...
fn select_target_binding(
    ...
    for comment in comments {
            ...
            .filter(|line| line.starts_with(MARKER_PREFIX))
        {
            let header = match parse_marker_header(line) {
                Ok(header) => header,
                Err(error) if header_targets_request(line, request, base) => {
                    malformed_targets.push((comment.id, error));
                    continue;
                }
                Err(_) => continue,
            };
    ...
    let Some(highest_sequence) = candidates.iter().map(|(sequence, _, _, _)| *sequence).max()
        ...
        if let Some((comment_id, error)) = malformed_targets.iter().max_by_key(|(id, _)| id) {
            return Err(format!(
    ...
    if let Some((malformed_id, error)) = malformed_targets
        .iter()
...
fn header_targets_request(line: &str, request: &TrustedProvenanceRequest, base: &str) -> bool {
    let tokens = line.split_ascii_whitespace().collect::<BTreeSet<_>>();
    tokens.contains(format!("target_repository={}", request.repository).as_str())
        && tokens.contains(format!("pull_request={}", request.pull_request).as_str())
        && tokens.contains(format!("base_sha={base}").as_str())
}
...
mod tests {
    ...
    fn unrelated_malformed_history_does_not_block_the_target() {
        let fixture = GitFixture::regular();
        let unrelated = GitHubIssueComment {
            id: 9,
            issue_url: issue_url(),
        ...
        let source = FakeCommentSource {
            comments: vec![unrelated, fixture.comment(10, 2)],
        };
