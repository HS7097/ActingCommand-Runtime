...
An active binding authorizes only its exact validation base, and its subject must be a commit in
the strict `(base, head]` range. A range with zero protected surface changes fails verification.
