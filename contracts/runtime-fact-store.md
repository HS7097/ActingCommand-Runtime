...
- `record` accepts a validated record. An identical record is idempotent
  (`Unchanged`); a newer observation replaces the stored one (`Updated`); an
  older observation is rejected (`Stale`) so a late writer can never roll a
  value back. Capacity overflow is an error, not an eviction.
- `invalidate` drops one record and returns the audit entry;
  ...
  families, used on owner takeover and device close.
- `snapshot(ledger_position, now)` returns the sealed image; `replay(snapshot)`
  replaces the store from one; `drain_pending` hands the host the records
  accepted since the last seal.
...
latest snapshot plus the records appended after it. Anything not appended or
sealed is gone with the process.
