...
| generated package validation used by `package build-task` | `crates/lab/src/package_build.rs:778` | The newly written temporary zip is still validated with a digest computed from those same local bytes before dry-run removal or final rename. No external hash source was added. |
