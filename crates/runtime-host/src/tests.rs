...
fn admitted_mapped_run_fixture(
    ...
    admitted_mapped_run_fixture_with_config(root, outcome_key, config(root))
}

fn admitted_mapped_run_fixture_with_config(
    root: &TempDir,
    outcome_key: &str,
    runtime_config: RuntimeHostConfig,
) -> (
    RuntimeHost,
    Arc<FakeState>,
    Box<PolicyRunContext>,
...
fn scheduled_policy_checkpoint_is_exact_thread_bound_one_shot_and_clock_independent() {
    ...
    let (host, _state, context, request) = admitted_mapped_run_fixture_with_config(
        &root,
        "checkpoint-exact-one-shot",
        config(&root).with_runtime_clock(Arc::clone(&clock) as Arc<dyn RuntimeClock>),
    );
    let receipt = host
        .run_scheduled_contained_task(&context, &request)
...
fn exact_lifecycle_checkpoint_recovers_scheduled_failure_once() {
    ...
    assert_eq!(
        prefix
            ...
            .count(),
        1,
        "durable release prefix"
    );
    assert!(
        prefix
            .iter()
            .all(|event| event.event_type() != EventType::PolicyExecutionRecorded),
        "checkpoint prefix already contains policy execution"
    );
    assert!(
        prefix
            .iter()
            .all(|event| event.event_type() != EventType::PolicyDispatchCompleted),
        "checkpoint prefix already contains policy completion"
    );
    let release = prefix
        .iter()
