...
impl HostShared {
    ...
    fn recognize_artifact_inner(
        ...
        let image = read_projected_verified(&root, &input.source.artifact)
            .map_err(|error| source_error(error.code(), error))?;
        let bundle = if matches!(&input.expected_sha256, actingcommand_contract::PackageRef::GitSourceTree(_)) {
            let provider = self.execution.vision_provider().ok_or_else(|| source_error("saved_ocr_provider_unavailable", "Runtime OCR provider is not configured"))?;
            ExternallyVerifiedBundle::load_path("saved_artifact_ocr", Path::new(&input.package_path), &input.expected_sha256, false, Some(provider), deadline)
        ...
        let expected = ExternalExpectedSha256::parse_hex(input.expected_sha256.legacy_sha256().ok_or_else(|| source_error("saved_ocr_package_hash_invalid", "legacy package identity missing"))?)
