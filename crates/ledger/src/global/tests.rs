...
fn signature_catalog_rebuilds_versions_and_binds_paginated_multiple_matches() {
    store_contract::signature_catalog_rebuilds_versions_and_binds_paginated_multiple_matches(
        GlobalLedger::open,
    );
}
...
fn store_open_failure_joins_waiting_writer() {
    let temp = TempDir::new().expect("temp");
    let error =
        GlobalLedger::open_with_store::<SegmentStore, _>(config(&temp, "writer-failed"), |_| {
...
fn scheduled_settlement_continuation_is_one_shot_and_idempotent() {
    store_contract::scheduled_settlement_continuation_is_one_shot_and_idempotent(
        GlobalLedger::open,
    );
}
...
fn query_pages_are_bounded_ordered_and_pinned_to_the_requested_snapshot() {
    store_contract::query_pages_are_bounded_ordered_and_pinned_to_the_requested_snapshot(
        GlobalLedger::open,
    );
}
...
fn subscription_reports_terminal_writer_failure() {
    store_contract::subscription_reports_terminal_writer_failure(GlobalLedger::open);
}
...
fn indexes_rebuild_after_reopen() {
    store_contract::indexes_rebuild_after_reopen(GlobalLedger::open);
}
