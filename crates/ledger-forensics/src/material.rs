...
use actingcommand_contract::{
    ...
    RuntimeMaterialReadSource, RuntimeMaterialReadState,
};
...
pub fn read_material_to<W: Write>(
    ...
    if let Err(error) = work {
            ...
            ReadError::Ledger(error) => {
                let (state, limit) = match error.code() {
                    ...
                    "ledger_source_incomplete" => {
                        (RuntimeMaterialReadState::SourceIncomplete, None)
                    }
        ...
        native_error = Some(ForensicError::new(code, operation, detail));
        result.state = state;
        result.limit = limit;
        result.chunk = None;
