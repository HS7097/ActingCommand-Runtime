...
fn selected_build_retains_required_ocr_target_closure() {
    ...
    source["schema_version"] = json!("0.8");
    source["entry_page"] = json!("any");
    source["target_page"] = json!(["selected-page", "lucky"]);
    source["error_pages"] = json!(["unknown-lucky"]);
    source["ocr_targets"][0]["region"]["anchor_target_id"] = json!("page/unselected-page");
    for page in ["lucky", "unknown-lucky"] {
        let mut anchor = source["anchors"][0].clone();
        anchor["id"] = json!(page);
        source["anchors"].as_array_mut().unwrap().push(anchor);
    }
    source["page_rules"] = json!({
        "selected-page":{"required":["page/selected-page"],"forbidden":["color/lucky", "color/unknown"]},
        "lucky":{"required":["page/lucky"],"any_of":[["color/lucky"]],"forbidden":["color/unknown"]},
        "unknown-lucky":{"required":["page/unknown-lucky", "color/unknown"]}
    });
    source["post_admission_ocr"] = json!({"mode":"fields_v1",
        "page_ids":["neutral/selected-page", "neutral/lucky"],
        "fields":[{"id":"count","group":"drop","target_id":"ocr/selected","required":false,
            "required_on_pages":["neutral/lucky"],"privacy":"public","trim":"whitespace_v1",
    ...
    source["scheduling_outcome"] = json!({"mappings":[{"outcome_key":"fields_recorded",
        "effect":"no_designated_effect","terminal_pages":["selected-page", "lucky"]}]});
    fs::write(&path, serde_json::to_vec(&source).unwrap()).unwrap();
    let donor_path = root.path().join("operations/unselected/task.json");
    let mut donor: Value = serde_json::from_slice(&fs::read(&donor_path).unwrap()).unwrap();
    donor["color_probes"] = json!([
        {"id":"color/lucky","region":{"mode":"rect","rect":{"x":0,"y":0,"width":1,"height":1}},"expected":[1,2,3]},
        {"id":"color/unknown","region":{"mode":"rect","rect":{"x":1,"y":0,"width":1,"height":1}},"expected":[3,2,1]}
    ]);
    fs::write(&donor_path, serde_json::to_vec(&donor).unwrap()).unwrap();
    let converter = OperationConverter::load(root.path(), None, None, None).unwrap();
    let selected = converter
        .build_selected(&["selected".into()])
        .expect("all collection and error page dependencies retained");
    let targets = selected.pack["targets"].as_array().unwrap();
    for target in [
        "page/selected-page",
        "page/lucky",
        "page/unknown-lucky",
        "page/unselected-page",
        "color/lucky",
        "color/unknown",
        "ocr/selected",
    ] {
        assert!(targets.iter().any(|row| row["id"] == target), "{target}");
    }
    assert!(!targets.iter().any(|row| row["id"] == "ocr/unselected"));
    let pages = selected.pages["pages"].as_array().unwrap();
    assert_eq!(pages.len(), 3);
    for page in ["selected-page", "lucky", "unknown-lucky"] {
        let generated = pages
            .iter()
            .find(|row| row["id"] == format!("neutral/{page}"))
            .unwrap();
        for field in ["required", "any_of", "forbidden"] {
            if let Some(value) = source["page_rules"][page].get(field) {
                assert_eq!(&generated[field], value, "{page}/{field}");
            }
        }
    }
    let generated = converter.canonical_task("selected").unwrap();
    assert_eq!(
        generated["post_admission_ocr"],
        source["post_admission_ocr"]
    );
    assert_eq!(generated["error_pages"], json!(["unknown-lucky"]));
}
