...
mod tests {
    ...
    #[test]
    fn page_projection_operation_privacy_controls_field_visibility() {
        let mut hashes = BTreeMap::new();
        for companion in 0..4 {
            for personal_source in 0..5 {
                let mut annotations = declaration();
                if companion & 1 == 0 && personal_source != 1 {
                    annotations.targets.clear();
                }
                if companion & 2 == 0 && personal_source != 2 {
                    annotations.fields.clear();
                }
                if personal_source == 1 {
                    annotations.targets[0].privacy = Privacy::Personal;
                }
                if personal_source == 2 {
                    annotations.fields[0].privacy = Privacy::Personal;
                }
                ...
                let metadata = if companion == 0 && personal_source == 0 {
                    ...
                    annotations.validate(references).unwrap()
                };
                let mut facts = input();
                facts.fields.push(FieldInput {
                    ...
                    value: Some(json!("sample value")),
                    detail: Some("sample detail".into()),
                    privacy: Some(if personal_source == 4 {
                        Privacy::Personal
                    } else {
                        Privacy::Public
                    }),
                ...
                if personal {
                    ...
                    assert_eq!(field["detail"], "sample detail");
                }
                if let Some(expected) = hashes.get(&personal) {
                    assert_eq!(&projected.content_sha256, expected);
                } else {
                    hashes.insert(personal, projected.content_sha256);
                }
            }
        }
    }
    ...
    #[test]
    fn page_projection_catalog_identity_controls_unannotated_values() {
        let unannotated = VerifiedProjectionMetadata::unannotated(catalog());
        let annotated = declaration().validate(catalog()).unwrap();
        for oversized in [false, true] {
            let mut hashes = Vec::new();
            for metadata in [&unannotated, &annotated] {
                let mut facts = input();
                facts.fields.push(FieldInput {
                    target_id: "value".into(),
                    field: None,
                ...
                let projected = project(facts, metadata).unwrap();
                assert_eq!(projected.metrics.entry_count, 1);
                assert_eq!(projected.output_truncated, oversized);
                assert_eq!(projected.omitted_count, usize::from(oversized));
                assert!(serialized_len(&projected).unwrap() <= BYTE_LIMIT);
                if oversized {
                    assert!(projected.fields.is_empty());
                } else {
                    assert_eq!(projected.fields[0]["redacted"], false);
                    assert_eq!(projected.fields[0]["privacy"], "public");
                    assert_eq!(projected.fields[0]["raw_text"], "sample raw");
                    assert_eq!(projected.fields[0]["value"], "sample value");
                    assert_eq!(projected.fields[0]["detail"], "sample detail");
                }
        ...
        for personal_source in 0..2 {
            let mut annotations = declaration();
            if personal_source == 0 {
                annotations.targets[0].privacy = Privacy::Personal;
            }
