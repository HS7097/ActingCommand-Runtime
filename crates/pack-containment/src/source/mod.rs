...
use actingcommand_recognition_pack::{AssetResolver, RecognitionPackError, RecognitionPackResult};
...
pub fn source_file_requests(bundles: &[Bundle]) -> BTreeMap<PathBuf, SourceRead> {
    ...
    for bundle in bundles {
        for key in ["anchors", "verify_templates"] {
            for declaration in array_field(&bundle.data, key) {
                let Some(template) = declaration.get("template").and_then(Value::as_str) else {
                    continue;
                };
        ...
        if bundle.data["schema_version"].as_str() == Some("0.8") {
            if let Some(fields) = bundle
                .data
                .get("post_admission_ocr")
                .and_then(|value| {
                    serde_json::from_value::<OcrFieldsDeclaration>(value.clone()).ok()
                })
                .filter(|fields| fields.validate().is_ok())
            {
                for field in &fields.fields {
                    if let OcrFieldType::DictionaryEntry { dictionary } = &field.value {
                        add(
                            bundle.dir.join(&dictionary.path),
                            SourceRead::BoundedBytes(fields.limits.max_total_bytes),
                        );
                    }
                }
            }
        } else if let Some(path) = bundle
            .data
            .pointer("/post_admission_ocr/truth_set/path")
            .and_then(Value::as_str)
            .filter(|path| safe_task_local_resource_path(path))
        {
            add(bundle.dir.join(path), SourceRead::Bytes);
        }
    }
    requests
}
...
impl ConversionFiles {
    ...
    fn length(&self, path: &Path) -> Result<u64, String> {
        self.files
            .get(path)
            .ok_or_else(|| format!("source metadata not supplied: {}", path.display()))?
            .length
            .clone()
    }
    fn read(&self, path: &Path) -> Result<&[u8], String> {
        self.files
            .get(path)
            .ok_or_else(|| format!("source bytes not supplied: {}", path.display()))?
            .bytes
            .as_deref()
            .map_err(Clone::clone)
    }
...
impl AssetResolver for SourceAssetResolver {
    fn read_asset(&self, path: &str) -> RecognitionPackResult<Vec<u8>> {
        self.files
            ...
            .map(<[u8]>::to_vec)
            .map_err(|error| {
                RecognitionPackError::fatal(format!("failed to read asset '{path}': {error}"))
            })
    }
    fn contains_asset(&self, path: &str) -> bool {
        self.files.is_file(&self.root.join(path))
    }
}
...
pub struct OperationConverter {
    ...
    pub existing_navigation: Option<Value>,
    pub maa_task_overlays: HashMap<String, Value>,
}
...
impl OperationConverter {
    ...
    pub fn build_all(&self, files: &ConversionFiles) -> CliOutcome<ConvertOutputs> {
        ...
        let bytes = match &files.projection_bytes {
            Ok(Some(bytes)) => bytes,
    ...
    fn build_resources_with_dependencies(
        ...
        let pack = if dependencies.is_empty() {
            ...
            self.build_pack_with_dependencies(dependencies, files)?
        };
        validate_pack_targets_exist(&self.root, &pack, files)?;
    ...
    pub fn build_selected(
        ...
        subset.validate_bundles(files)?;
        let mut outputs = subset.build_resources_with_dependencies(&dependencies, files)?;
        let annotation_path = self
            .root
            .join("navigation")
            .join(format!("{}.{}.projection.json", self.game, self.server));
        if !*files.projection_exists.as_ref().map_err(|e| {
            CliError::package_invalid(format!(
                "failed to inspect {}: {e}",
    ...
    pub fn canonical_task(&self, task_id: &str) -> CliOutcome<Value> {
        let bundle = self
            .bundles
            .iter()
    ...
    pub fn prune_page_rules_for_selected_build(
        &self,
        bundles: Vec<Bundle>,
        dependencies: &[Bundle],
    ...
    pub fn validate_bundles(&self, files: &ConversionFiles) -> CliOutcome<()> {
        ...
        for bundle in &self.bundles {
            ...
            if let Err(error) = validate_post_admission_ocr_bundle(bundle, files) {
                errors.push(error.message);
            }
            ...
            for anchor in array_field(&bundle.data, "anchors") {
                ...
                if !files.is_file(&bundle.dir.join(&template)) {
                    errors.push(format!(
            ...
            for verify_template in array_field(&bundle.data, "verify_templates") {
                ...
                } else if !files.is_file(&bundle.dir.join(&template)) {
                    errors.push(format!(
    ...
    pub fn build_navigation(&self) -> CliOutcome<Value> {
    ...
    pub fn build_primitives(&self) -> CliOutcome<Value> {
...
impl Bundle {
    pub fn task_json_path(&self) -> PathBuf {
        self.dir.join("task.json")
    }
}

pub fn resource_ids(resources: &Value) -> CliOutcome<HashSet<String>> {
...
pub fn validate_phases_bundle(bundle: &Bundle) -> CliOutcome<()> {
...
pub fn validate_post_admission_ocr_bundle(
    ...
    if schema == "0.8" {
        ...
        validate_ocr_fields_mode_declaration(schema, value, bundle.data.get("scheduling_outcome"))?;
        ...
        for field in &fields.fields {
            ...
            if let OcrFieldType::DictionaryEntry { dictionary } = &field.value {
                ...
                let size = files.length(&path).map_err(CliError::package_invalid)?;
                ...
                let bytes = files.read(&path).map_err(CliError::package_invalid)?;
    ...
    let truth_bytes = files.read(&bundle.dir.join(truth_path)).map_err(|error| {
        CliError::package_invalid(format!(
...
fn validate_pack_targets_exist(
    ...
    for target in array_field(pack, "targets") {
        ...
        if !files.is_file(&root.join(path)) {
            errors.push(format!(
...
pub fn required_string(value: &Value, key: &str) -> CliOutcome<String> {
    value
        .get(key)
        .and_then(Value::as_str)
        .map(str::to_string)
...
pub fn first_server_scope(bundle: &Value) -> Option<String> {
    bundle
        .get("server_scope")
        .and_then(Value::as_array)
...
pub fn validate_ocr_fields_mode_declaration(
    schema: &str,
    value: &Value,
