...
fn pre_execution_guard_passes_when_page_and_target_match() {
    ...
    let outcome = evaluate_pre_execution_guard("arknights", &operation, guard, &scene, &evaluator)
...
fn resource_drift_gate_rejects_initial_passing_target() {
    let err = ResourceDriftGate::new(2, color_target_evaluation("target/button", [0, 0, 0], true))
...
fn operation_verification_marks_to_null_without_template_unverified() {
    ...
    let result =
        operation_verification_status("arknights", &operation, &[], &scene).expect("verification");
...
fn each_finite_destination_can_verify_independently() {
    ...
    for page in ["arknights/alternate", "arknights/terminal"] {
        ...
        assert_eq!(
            post_action_page_status("arknights", &operation, &[], &scene).expect("page status"),
...
fn multiple_destinations_in_one_observation_fail_without_order_selection() {
    ...
    let scene = captured_scene_with_matches(&["arknights/terminal", "arknights/alternate"], false);
...
fn destination_before_error_in_full_evaluations_still_fails_closed() {
    let operation = test_operation(Some("terminal"), None);
    let scene =
        captured_scene_with_matches(&["arknights/terminal", "arknights/error_popup"], false);
