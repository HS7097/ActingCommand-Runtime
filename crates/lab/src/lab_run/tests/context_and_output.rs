...
fn failure_zip_materializes_frame_store_screenshots() {
    ...
    assert!(ledger.records.iter().any(|record| {
        record.kind == LedgerRecordKind::Drive
            && record.payload.get("record_type").and_then(Value::as_str) == Some("evidence_index")
    }));
    assert!(ledger.records.iter().any(|record| {
        record.kind == LedgerRecordKind::Drive
...
fn recognition_projection_keeps_reco_id_from_ledger() {
    ...
    let evidence_bytes = fs::read(temp.path().join(evidence_ref)).expect("recognition evidence");
    ...
    let record = ledger
        ...
        .find(|record| {
            record.kind == LedgerRecordKind::Drive
                && record.payload.get("record_type").and_then(Value::as_str) == Some("recognition")
        })
...
fn success_finish_cleans_run_dir_but_keeps_outside_zip() {
    ...
    assert!(!ctx.run_dir.exists());
    assert!(ctx.ledger_path.as_ref().expect("ledger path").is_file());
    let ledger_text = fs::read_to_string(ctx.ledger_path.as_ref().unwrap()).expect("ledger text");
    assert_ordered(
        &ledger_text,
        &[
